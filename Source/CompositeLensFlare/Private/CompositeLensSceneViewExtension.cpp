...
TAutoConsoleVariable<int32> CVarLensFlareEnabled(
	...
	{
		if (GEngine && GEngine->IsInitialized() && FModuleManager::Get().IsModuleLoaded("CustomLensFlare"))
		{
			FCompositeLensFlareModule& CustomLensFlareModule = FModuleManager::Get().GetModuleChecked<FCompositeLensFlareModule>("CustomLensFlare");
			if (Value->GetInt() != 0)
			{
				CustomLensFlareModule.SetupLensFlare();
			}
			else
			{
				CustomLensFlareModule.DestroyLensFlare();
			}
		}
	}),
	ECVF_RenderThreadSafe);

DECLARE_GPU_STAT(CustomLensFlare);
...
namespace
{
	...
	IMPLEMENT_GLOBAL_SHADER(FCustomScreenPassVS, "/Plugin/CompositeLensFlare/ScreenPass.usf",
	...
	IMPLEMENT_GLOBAL_SHADER(FLensFlareRescalePS, "/Plugin/CompositeLensFlare/Rescale.usf", "RescalePS", SF_Pixel);
	...
	IMPLEMENT_GLOBAL_SHADER(FDownsamplePS, "/Plugin/CompositeLensFlare/DownsampleThreshold.usf", "DownsampleThresholdPS",
	                        SF_Pixel);
	...
	IMPLEMENT_GLOBAL_SHADER(FKawaseBlurDownPS, "/Plugin/CompositeLensFlare/DualKawaseBlur.usf", "KawaseBlurDownsamplePS",
	                        SF_Pixel);
	IMPLEMENT_GLOBAL_SHADER(FKawaseBlurUpPS, "/Plugin/CompositeLensFlare/DualKawaseBlur.usf", "KawaseBlurUpsamplePS",
	                        SF_Pixel);
	...
	IMPLEMENT_GLOBAL_SHADER(FLensFlareChromaPS, "/Plugin/CompositeLensFlare/Chroma.usf", "ChromaPS", SF_Pixel);
	...
	IMPLEMENT_GLOBAL_SHADER(FLensFlareGhostsPS, "/Plugin/CompositeLensFlare/Ghosts.usf", "GhostsPS", SF_Pixel);
	...
	IMPLEMENT_GLOBAL_SHADER(FLensFlareHaloPS, "/Plugin/CompositeLensFlare/Halo.usf", "HaloPS", SF_Pixel);
	...
	IMPLEMENT_GLOBAL_SHADER(FLensFlareGlareVS, "/Plugin/CompositeLensFlare/Glare.usf", "GlareVS", SF_Vertex);
	IMPLEMENT_GLOBAL_SHADER(FLensFlareGlareGS, "/Plugin/CompositeLensFlare/Glare.usf", "GlareGS", SF_Geometry);
	IMPLEMENT_GLOBAL_SHADER(FLensFlareGlarePS, "/Plugin/CompositeLensFlare/Glare.usf", "GlarePS", SF_Pixel);
	...
	IMPLEMENT_GLOBAL_SHADER(FLensFlareBloomMixPS, "/Plugin/CompositeLensFlare/Mix.usf", "MixPS", SF_Pixel);
}

FCompositeLensSceneViewExtension::FCompositeLensSceneViewExtension(const FAutoRegister& AutoRegister) : FSceneViewExtensionBase(AutoRegister)
{
	UE_LOG(LensSceneViewExtensionLog, Display, TEXT("Composite Lens View Extension Ctor."));
}

FCompositeLensSceneViewExtension::~FCompositeLensSceneViewExtension()
{
}

bool FCompositeLensSceneViewExtension::IsActiveThisFrame_Internal(const FSceneViewExtensionContext& Context) const
{
	return DefaultConfig.IsValid();
}

void FCompositeLensSceneViewExtension::SetupViewFamily(FSceneViewFamily& InViewFamily)
{
	FSceneViewExtensionBase::SetupViewFamily(InViewFamily);
}

void FCompositeLensSceneViewExtension::SetupView(FSceneViewFamily& InViewFamily, FSceneView& InView)
{
	FSceneViewExtensionBase::SetupView(InViewFamily, InView);
}

void FCompositeLensSceneViewExtension::BeginRenderViewFamily(FSceneViewFamily& InViewFamily)
{
	// Check world
	...
	for (const FSceneView* CurrentView : InViewFamily.Views)
	{
		...
		const FVector ViewLocation = CurrentView->ViewLocation;

		float BestPriority = -FLT_MAX;
		UCompositeLensFlareConfig* BestConfig = nullptr;

		for (IInterface_PostProcessVolume* VolumeInterface : World->PostProcessVolumes)
		{
			...
			if (bIsUnbound || PPVolume->EncompassesPoint(ViewLocation, 0.f, nullptr))
			{
				if (PPVolume->Priority > BestPriority)
				{
					...
					BestConfig = LensFlareComponent->CompositeLensFlareConfig;
				}
			}
		}
		...
		if (BestConfig)
		{
			Config = TStrongObjectPtr<UCompositeLensFlareConfig>(BestConfig);
		}
		else
		{
...
void FCompositeLensSceneViewExtension::Initialize()
{
	FString DefaultConfigPath = TEXT("/CompositeLensFlare/DA_LensFlareConfig.DA_LensFlareConfig");
	UCompositeLensFlareConfig* LoadedConfig = LoadObject<UCompositeLensFlareConfig>(nullptr, *DefaultConfigPath);
	if (LoadedConfig)
	{
		DefaultConfig = TStrongObjectPtr<UCompositeLensFlareConfig>(LoadedConfig);
		Config = DefaultConfig;
...
FScreenPassTexture FCompositeLensSceneViewExtension::AddLensFlareDOFPass_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& View, const FPostProcessMaterialInputs& InOutInputs)
{
	if (!Config.IsValid())
	{
...
void FCompositeLensSceneViewExtension::RenderLensFlare(FRDGBuilder& GraphBuilder, const FViewInfo& View, FScreenPassTexture BloomTexture, FScreenPassTextureSlice HalfSceneColor, FScreenPassTexture& Outputs)
{
	if (!Config.IsValid())
	{
		return;
...

	{
		...
		FIntRect MixViewport = BloomTexture.ViewRect;
		...
		Description.Reset();
		Description.Extent = BloomTexture.Texture->Desc.Extent;
...
FRDGTextureRef FCompositeLensSceneViewExtension::RenderThreshold(FRDGBuilder& GraphBuilder, FRDGTextureRef InputTexture, FIntRect& InputRect, const FViewInfo& View)
{
...
FRDGTextureRef FCompositeLensSceneViewExtension::RenderFlare(FRDGBuilder& GraphBuilder, FRDGTextureRef InputTexture, FIntRect& InputRect, const FViewInfo& View)
{
...
FRDGTextureRef FCompositeLensSceneViewExtension::RenderGlare(FRDGBuilder& GraphBuilder, FRDGTextureRef InputTexture, FIntRect& InputRect, const FViewInfo& View)
{
...
FRDGTextureRef FCompositeLensSceneViewExtension::RenderBlur(FRDGBuilder& GraphBuilder, FRDGTextureRef InputTexture, const FViewInfo& View, const FIntRect& Viewport, int BlurSteps)
{
	// Shader setup
	TShaderMapRef<FCustomScreenPassVS> VertexShader(View.ShaderMap);
